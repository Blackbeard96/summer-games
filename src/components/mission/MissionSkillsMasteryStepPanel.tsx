...
const MissionSkillsMasteryStepPanel: React.FC<Props> = ({
  step,
  userId,
  missionId,
  playerMissionId,
  ...
  const openSkillsMastery = async () => {
    ...
    try {
      ...
      console.error('Skills mastery grant/visit failed', e);
      grantError = e instanceof Error ? e.message : 'Could not apply PP grant.';
      // Still mark visited (best effort) so Next can unlock, then send the player to Skills & Mastery.
      try {
        await markSkillsMasteryStepVisited(pmId, step.id);
      } catch (visitErr) {
        console.warn('Skills mastery mark visited failed', visitErr);
      }
    }
    ...
    // Navigate regardless of grant outcome so the step is never a dead end.
    navigate(`/battle?${q.toString()}#moves`);

    if (grantError) {
      // Shown briefly if navigate is slow/blocked; primary path is leave this page.
...
export default MissionSkillsMasteryStepPanel;
